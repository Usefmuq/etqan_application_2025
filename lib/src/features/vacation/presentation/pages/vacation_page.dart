...
class _VacationPageState extends State<VacationPage>
  ...
  void _setupTabsAndController() {
    ...
    setState(() {
      _tabKinds = kinds;
      bodyPerTab = List.generate(_tabKinds.length, (_) => [_vacationTab()]);
  ...
  Widget build(BuildContext context) {
    return CustomScaffold(
      title: AppLocalizations.of(context)!.vacationsService,
      subtitle: AppLocalizations.of(context)!.vacationsServiceSubtitle,
      tabController: _tabController,
  ...
  Widget _vacationTab() {
    return BlocConsumer<VacationBloc, VacationState>(
      ...
      builder: (context, state) {
        ...
        if (state is VacationShowAllSuccess) {
          if (state.vacationPage.vacationsView.isEmpty) {
          ...
          return ListView.builder(
            ...
            itemCount: state.vacationPage.vacationsView.length,
            itemBuilder: (context, index) {
              final vacation = state.vacationPage.vacationsView[index];
              ...
              return Padding(
                ...
                child: AnimatedCardWrapper(
                  ...
                  child: CustomCardListRequests(
                    title:
                        '${Localizations.localeOf(context).languageCode == 'ar' ? vacation.vacationTypeAr : vacation.vacationTypeEn} ${AppLocalizations.of(context)!.vacationDaysCount}: ${vacation.daysCount} ',
                    statusId: vacation.requestStatusId,
                    requestDate: vacation.requestCreatedAt,
                    subtitle: '${vacation.startDate} - ${vacation.endDate}',
                    onTap: () {
                      context.push(
                        '/vacations/${vacation.requestId}',
                        extra: VacationViewerPageEntity(
                          vacationsView: vacation,
                          approval: state.vacationPage.approvalsView
                              .where((a) => a.requestId == vacation.requestId)
                              .toList(),
                        ),
                      );
                    },
