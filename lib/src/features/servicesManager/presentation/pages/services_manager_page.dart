...
class ServicesManagerPage extends StatefulWidget {
  static route() => MaterialPageRoute(
        builder: (context) => const ServicesManagerPage(),
      );

  const ServicesManagerPage({super.key});

  @override
  State<ServicesManagerPage> createState() => _ServicesManagerPageState();
}

class _ServicesManagerPageState extends State<ServicesManagerPage>
  ...
  void _setupTabsAndController() {
    ...
    setState(() {
      _tabKinds = kinds;
      bodyPerTab =
          List.generate(_tabKinds.length, (_) => [_servicesmanagerTab()]);
  ...
  Widget build(BuildContext context) {
    return CustomScaffold(
      title: AppLocalizations.of(context)!.servicesManagerService,
      subtitle: AppLocalizations.of(context)!.servicesManagerServiceSubtitle,
      tabController: _tabController,
      ...
      tilteActions: [
        if (isUserHasPermissionsView(
          permissions ?? [],
          PermissionsConstants.addServicesManager,
        ))
          IconButton(
            onPressed: () {
              context.push('/servicesmanager/submit/');
            },
  ...
  Widget _servicesmanagerTab() {
    return BlocConsumer<ServicesManagerBloc, ServicesManagerState>(
      ...
      builder: (context, state) {
        ...
        if (state is ServicesManagerLoading || !permsReady) {
          return const Loader();
        }
        if (!canAdd) {
          return Center(
        ...
        if (state is ServicesManagerShowAllSuccess) {
          if (state.servicesmanagerPage.servicesmanagersView.isEmpty) {
          ...
          return ListView.builder(
            ...
            itemCount: state.servicesmanagerPage.servicesmanagersView.length,
            itemBuilder: (context, index) {
              final servicesmanager =
                  state.servicesmanagerPage.servicesmanagersView[index];
              ...
              return Padding(
                ...
                child: AnimatedCardWrapper(
                  ...
                  child: CustomCardListRequests(
                    chips: servicesmanager.topics ?? [],
                    title: servicesmanager.title ?? '',
                    statusId: servicesmanager.requestStatusId,
                    requestDate: servicesmanager.requestCreatedAt,
                    subtitle: servicesmanager.content,
                    onTap: () {
                      context.push(
                        '/servicesmanager/${servicesmanager.requestId}',
                        extra: ServicesManagerViewerPageEntity(
                          servicesmanagersView: servicesmanager,
                          approval: state.servicesmanagerPage.approvalsView
                              .where((a) =>
                                  a.requestId == servicesmanager.requestId)
                              .toList(),
                        ),
                      );
                    },
